...
export async function setVisionProxyModel(): Promise<void> {
  ...
  if (candidates.length === 0) {
    vscode.window.showInformationMessage(
      vscode.l10n.t('No vision-capable models found. Sign in to GitHub Copilot Chat to enable vision proxy.'),
    );
    return;
  }
  ...
  vscode.window.showInformationMessage(vscode.l10n.t('Vision proxy model set to: {0}', picked.label));
}
